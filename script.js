  ...
  */
  ...
  if ($('.user_info_section #user').length) {
   ...
   /* if ($("#user-nav").children().length === 0) {
    ...
    $(".header_container .toggle-search").click(function () {
        $('.header_container .header_search').addClass('active');
    });
  
    $(".header_container .icon-close").click(function () {
        $('.header_container .header_search').removeClass('active');
    });
  ...
  $(".article-sidebar li").on('click', function() {
    var links = $(".article-body").find("h2");
  	...
  	$('html, body').animate({
        scrollTop: links.eq(i).offset().top - 130}, 500);
  	});
...
function article_titles(){
  ...
  if($(links).length) {
      ...
      $('.article-sidebar .sticky-sidebar').append($('<ul/>'));
      links.each(function() {
        $('.article-sidebar .sticky-sidebar ul').append($('<li/>', {text: $(this).text()}));
      });
...
$(window).on("resize", function () {
  ...
  if ($('.footer_widget_nav_menu > div').length) {
      if (matchMedia('screen and (min-width: 1260px)').matches) {
          ...
          $("footer .footer_menu .widget-title").removeClass("active");
      }
  }
});
